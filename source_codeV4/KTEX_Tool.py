...
class KTEXConverter:
    ...
    def _load_metadata(self, png_path: Path, original_ktex: Optional[Path]) -> Tuple[Optional[bytes], dict]:
        ...
        meta = {}
            ...
            # === أولاً: البحث عن ملف TEX الأصلي بنفس الاسم ===
        ...
        if original_tex.exists():
                ...
                print(f"      Mipmaps: {meta['mipmap_count']} levels" if meta['has_mipmaps'] else "      Mipmaps: None")
        
                return header_data, meta
    
            # === ثانياً: إذا تم تحديد ملف أصلي يدوياً ===
        if original_ktex:
        ...
        return header_data, meta
            ...
            # === ثالثاً: ملفات الـ metadata المحفوظة ===
        ...
        if json_file.exists():
            ...
            # === تحذير إذا لم يجد أي مصدر ===
        if not header_data and not meta:
