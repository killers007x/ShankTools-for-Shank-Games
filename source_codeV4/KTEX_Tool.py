...
import time
import argparse
import threading                          # [FIX] Added for thread-safe printing
from pathlib import Path
from dataclasses import dataclass         # [FIX] Removed unused 'field'
from typing import List, Tuple, Optional  # [FIX] Removed unused 'Dict'
...
KTEX_MAGIC = b'KTEX'
_print_lock = threading.Lock()  # [FIX] Thread-safe console output for batch mode
...
class DXTFormat(IntEnum):
    ...
    def name_str(self) -> str:
        # [FIX] Replaced confusing chained ternary with clean index lookup
...
class KTEXConverter:
    ...
    def _load_metadata(self, png_path: Path, original_ktex: Optional[Path]) -> Tuple[Optional[bytes], dict]:
        """Load header and metadata from original TEX file.

        Priority order:
          1. .tex file with the same name in the same folder
          2. Manually specified file via --original
          3. Saved .ktex_header / .ktex_meta.json files
          4. Defaults (warns the user)
        """
        header_data = None
        meta = {}

        # ── 1. TEX file with the same name ────────────────────────────────────
        ...
        if original_tex.exists():
            ...
            return header_data, meta  # [FIX] Correct indentation + early return

        # ── 2. Manually specified original file ────────────────────────────────
        if original_ktex:                             # [FIX] Was unreachable before
            ...
            if original_ktex.exists():
                ...
                return header_data, meta  # [FIX] Added missing return

        # ── 3. Saved metadata files ────────────────────────────────────────────
        header_file = png_path.with_suffix('.ktex_header')  # [FIX] Was unreachable
    ...
    def _create_header(self, width: int, height: int, fmt: DXTFormat,
        ...
        if has_mipmaps and version == 8:
            ...
            # Compact mipmap header (10 bytes — no room for height field)
            header = bytearray(10)
            header[0:4] = KTEX_MAGIC
            header[6] = version
            header[7] = int(fmt)
    ...
    def batch_extract(self, files: List[Path], output_dir: Optional[Path] = None,
        ...
        """Extract multiple files in parallel"""  # [FIX] Now actually parallel
        if output_dir:
    ...
    def batch_rebuild(self, files: List[Path], output_dir: Optional[Path] = None,
        ...
        """Rebuild multiple files in parallel"""  # [FIX] Now actually parallel
        if output_dir:
            output_dir = Path(output_dir)
            output_dir.mkdir(parents=True, exist_ok=True)
...
╚══════════════════════════════════════════════════════════════════════════════╝
    ...
    if args.command == 'extract':
        if len(input_files) == 1 and args.output and not Path(args.output).suffix:
            ...
            # [FIX] Pass extract_all_mipmaps flag to batch method
        ...
        if len(input_files) == 1:
            ...
            output_dir = Path(args.output) if args.output else None
            # [FIX] Pass force_mipmaps flag to batch method
